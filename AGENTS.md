...
1. **先读** `docs\面试准备\面试准备_上下文.md`（两个项目的经历素材 + 答题框架 + 模拟面试用法）
2. **跨项目取数**：视频增长经历/数据在 `D:\video-workflow\`（01_交接速览.md、抖音数据\作品列表*.xlsx）；简历工具项目本体在本仓库
3. **模拟面试**：优先引导用户用本工具"模拟面试" Tab（真实产品闭环）；也可 AI 直接对话式扮演面试官
4. **生成简历**：用 AI 产品经理 JD 走生成链路（含行业侧重点分析）
5. 面试准备产出物（可选）：经历库录入这段经历、AI产品经理定制简历、模拟面试记录